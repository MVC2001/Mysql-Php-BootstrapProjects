...
function addRole($connect, $category, $description) {
    // Check if role already exists
    $checkRoleQuery = mysqli_query($connect, "SELECT * FROM `roles` WHERE `category` = '$category'");
    if (mysqli_num_rows($checkRoleQuery) > 0) {
        return "Role already exists.";
    }

    // Insert role into database
    $insertRoleQuery = "INSERT INTO `roles` (`category`, `description`) VALUES ('$category', '$description')";
...
if (isset($_POST['add-function'])) {
    ...
    $result = addRole($connect, $category, $description);
