...
// Fetch record for the provided ID
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["id"])) {
  $id = $_GET["id"];
  $sql = "SELECT * FROM alphabet WHERE id = $id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["update"])) {
  ...
  if ($_FILES["alphabet"]["name"]) {
    $targetDir = "uploads/";
    $fileName = basename($_FILES["alphabet"]["name"]);
    ...
    if (in_array($fileType, $allowTypes)) {
      ...
      if (move_uploaded_file($_FILES["alphabet"]["tmp_name"], $targetFilePath)) {
        // Update record with new image and description
        $sql = "UPDATE alphabet SET alphabet='$fileName', description='$description' WHERE id=$id";
    ...
    // If no new image uploaded, only update description
    $sql = "UPDATE alphabet SET description='$description' WHERE id=$id";
    if ($connect->query($sql) === TRUE) {
      echo "Record updated successfully. <a href='all_alphabet.php'>Go back</a>";
    } else {
      echo "Error updating record: " . $connect->error;
    }
  }
...
<body class="app sidebar-mini rtl">
  ...
  <main class="app-content">
    <div class="app-title">
      ...
      <div class="col-md-12">
        <div class="tile">
          <h3 class="tile-title">Update alphabet here</h3>
          <hr>
          </hr>
          <div class="tile-body">
            <form action="" method="post" enctype="multipart/form-data">
              ...
              <div class="form-group">
                ...
                <input type="file" class="form-control-file" id="image" name="alphabet">
                <img src="uploads/<?php echo $image; ?>" alt="Current Image" style="max-width: 200px; max-height: 200px;">
              </div>
              <button type="submit" class="btn btn-primary" name="update">Update</button>
              <span><a href="./all_alphabet.php"><button type="button" class="btn btn-danger">Cancel</button></a></span>
            </form>
          </div>
        </div>
      </div>
