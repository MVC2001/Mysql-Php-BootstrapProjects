    ...
    <div class="d-flex">
        <nav class="sidebar sidebar-sticky" style="background-color: #2E4053; color: white;">
            <ul class="nav flex-column" style="margin-top: 70px;">
                <li class="nav-item">
                    <a class="nav-link active text-white" href="./newQuestons.php" style="font-size:21px">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card text-center" style="height:100px; box-shadow: rgba(60, 64, 67, 0.3) 0px 1px 2px 0px, rgba(60, 64, 67, 0.15) 0px 2px 6px 2px;">
                            <hr>
                            <i class="fa fa-users"></i> New Questions
                            <?php
                            $user_id = $_SESSION['user_id'];
                            $query = "
                                SELECT q.question_id, q.question, q.program, q.description, q.created_at
                                FROM questions q
                                JOIN user u ON q.program = u.program
                                LEFT JOIN (
                                ...
                                WHERE u.user_id = '$user_id' AND (a.answer_count IS NULL OR a.answer_count < 3)
                                ORDER BY q.question_id DESC
                                LIMIT 5";
                            $countv = mysqli_query($connect, $query) or die(mysqli_error($connect));
                            $countv_num = mysqli_num_rows($countv);
                            if ($countv_num > 0) {
                                echo "<div class='stat-digit'>$countv_num</div>";
                            } else {
                                echo "<div class='stat-digit'>0</div>";
                            }
                            ?>
                        </div>
                    </div>
