...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['user_id']) && !empty($_POST['user_id'])) {
        ...
        $deleteQuery = mysqli_query($connect, "DELETE FROM `user` WHERE `user_id` = $user_id");
        
        if ($deleteQuery) {
            // User deleted successfully
            header('Location: allUsers.php'); // Redirect to users page
            exit();
        } else {
            // Error deleting user
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4 mt-5">
            <div class="container shadow-sm">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h1>All users</h1>
                    <a href="./addUser.php"><button type="button" class="btn btn-secondary" id="addQuestionButton" style="background-color:#2E4053;">Add user</button></a>
                </div>
                ...
                <table class="table table-bordered table-striped">
                    ...
                    <tbody id="questionsTableBody">
                            <?php 
                 $count=1;
                 $select_all_users = "SELECT * FROM `user` ORDER BY user_id DESC";
